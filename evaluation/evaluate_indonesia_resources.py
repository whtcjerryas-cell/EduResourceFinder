...
参考Indonesia项目的网络配置，确保公司内部API可以正常访问
"""
...
class ResourceEvaluator:
    ...
    def __init__(self):
        ...
        timeout_config = httpx.Timeout(
            ...
            read=120.0,    # 读取超时120秒（AI评估可能需要更长时间）
            write=30.0,    # 写入超时30秒
            pool=10.0      # 连接池超时10秒
        )

        # 使用OpenAI SDK创建客户端，关键参数：
        # - proxy=None: 明确禁用代理
        # - trust_env=False: 不读取环境变量中的代理设置（非常重要！）
        self.client = OpenAI(
...
- 用中文撰写，专业且具体，不少于200字"""
        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                temperature=0.3,
                max_tokens=8000,  # 增加到8000，确保AI生成完整的评估
                timeout=120
            )
            ...
            print(f"   [DEBUG] JSON前500字符: {json_text[:500]}")
